...
const ExportCanvas = ({ nodes, connections, onRender }) => {
  ...
  const renderCanvas = useCallback(() => {
    ...
    const scale = 2
    canvas.width = Math.max(width * scale, 100)
    ...
    nodes.forEach(node => {
      ...
      ctx.quadraticCurveTo(parent.x + dr, parent.y, (parent.x + node.x) / 2, (parent.y + node.y) / 2)
    ...
    connections.forEach(conn => {
      ...
      ctx.quadraticCurveTo(from.x + dr, from.y, (from.x + to.x) / 2, (from.y + to.y) / 2)
    ...
    nodes.forEach(node => {
      ...
      // Вычисляем высоту узла на основе текста
      const text = node.text || 'Новый узел'
      const lines = text.split('\n')
      const lineCount = lines.length
      const lineHeight = fontSize * 1.2
      const nodeHeight = Math.max(lineHeight * lineCount + padding * 2, fontSize + padding * 2)
      
      const bgColor = node.style?.backgroundColor || (node.parentId ? '#1e293b' : '#3b82f6')
      const borderWidth = node.style?.borderWidth || 0
      const borderColor = borderWidth > 0 ? (node.style?.borderColor || '#3b82f6') : bgColor
      ...
      if (node.style?.shape === 'circle') {
        ...
        if (borderWidth > 0) {
          ...
          ctx.fillRect(-size / 2 - borderWidth/2, -size / 2 - borderWidth/2, size + borderWidth, size + borderWidth)
        }
      ...
      ctx.font = `${fontStyle}${fontWeight}${fontSize}px ${node.style?.fontFamily || 'Inter, sans-serif'}`
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
    ...
    // Сообщаем родителю что canvas готов
    if (!renderedRef.current) {
      renderedRef.current = true
      onRender(canvas)
    }
  }, [nodes, connections, onRender])

  // Рендерим при изменении данных
  useEffect(() => {
